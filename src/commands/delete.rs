...
fn run() -> Result<()> {
    ...
    if profiles.is_empty() {
        println!(
            "No git configuration profiles found to delete in {:?}.",
            config_dir
        );
        return Ok(());
    }
    ...
    let selection = Select::new(
        &format!(
            "Select Git Config profile to delete (Current: {})",
    ...
    if selection == current_profile_name {
        if git_config_path.exists() || fs::symlink_metadata(&git_config_path).is_ok() {
            fs::remove_file(&git_config_path)
                .context("Failed to remove current .gitconfig symlink")?;
        }
        println!(
            "{} Profile {:?} deleted. Current .gitconfig was also removed.",
            "Success:".green(),
            selection
        );
    } else {
